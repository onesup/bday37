class Admin::ViralActionsController < ApplicationController
  ...
  def index
    ...
    @viral_action_counts_daily = ViralAction.select("date(viral_actions.created_at) as created_date
    ...
    @viral_action_counts_sum = ViralAction.select("sum(case when viral_actions.device = 'pc' then 1 else 0 end) as pc_count,sum(case when viral_actions.device = 'mobile' then 1 else 0 end) as mobile_count, 
      	count(*) as total_count")
    @viral_platform_counts_daily = ViralAction.select("date(viral_actions.created_at) as created_date
